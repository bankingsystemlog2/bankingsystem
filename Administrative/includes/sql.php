...
/*--------------------------------------------------------------*/
/*  Function for Find data from request contract
/*--------------------------------------------------------------*/
function find_by_req_id($table,$id)
{
  global $db;
  $id = (int)$id;
    if(tableExists($table)){
          $sql = $db->query("SELECT * FROM {$db->escape($table)} WHERE req_id='{$db->escape($id)}' LIMIT 1");
...
/*--------------------------------------------------------------*/
 ...
 /*--------------------------------------------------------------*/
  ...
  function find_all_user(){
      ...
      $sql .="ON g.group_level=u.user_level WHERE u.department='administrative' ORDER BY u.name ASC";
            ...
            $sql = $db->query("SELECT p.*, s.Name FROM $table p LEFT JOIN status s ON s.Status_Code = p.Co_Status WHERE p.Co_Code='$name';");
              if($result = $db->fetch_assoc($sql))
                ...
                function  find_collection_transactions($table, $name){
                  ...
                  /*--------------------------------------------------------------*/
            /* select Calling all pending Finance disbursments
            /*--------------------------------------------------------------*/
          function  Call_Pending_Request(){
              global $db;
              $sql = "SELECT b.*, s.name AS NAME FROM budgetreleasing b INNER JOIN status s ON s.Status_Code = b.P_Status WHERE P_Status='106';" or die($mysqli->error);
              return find_by_sql($sql);
            }
