...
// $result = find_vendor_by_id('audit',$users_id);
 ...
 $all_request = find_all('core1files');
?>
<?php
  if(isset($_POST['applicationform'])){
   ...
   $imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
     $folder = '../Core1/contract/'.basename($_FILES["fileToUpload"]["name"]);
     $dl = 'contract/'.basename($_FILES["fileToUpload"]["name"]);
   ...
   if(empty($errors)){
       ...
       $body   = remove_junk($db->escape($_POST['body']));
        ...
        $query .=") VALUES (";
        $query .="'{$id}','{$title}', '{$sectiondep}', '{$datecreated}', '{$prepared_by}', '{$datefrom}', '{$dateto}', '{$body}','{$dl}', 'Downloaded'";
        $query .=")";
        ...
        if($db->query($query)){
          ...
          $_SESSION['status'] =  "Succesful Forward Files";
...
<a href="#.php"><button type="button" class="btn btn-light">CORE 2</button></a>
...
<div class="panel-body">
      ...
      <head>
        ...
        <div id="ApplicationForm" class="tabcontent">
            <form method="post" action="core 1 contract.php" autocomplete="off" enctype="multipart/form-data"> 
