...
/*--------------------------------------------------------------*/
 ...
 /*--------------------------------------------------------------*/
  ...
  function find_all_user(){
      ...
      $sql .="ON g.group_level=u.user_level WHERE u.Department='Finance' ORDER BY u.name ASC";
      ...
      function  notification_Loans(){
        ...
        function  notification_Budget(){
           ...
           function collections(){
             global $db;
             $sql = "SELECT * FROM collection;" or die($mysqli->error);
            ...
            $sql = $db->query("SELECT p.*, s.Name FROM $table p LEFT JOIN status s ON s.Status_Code = p.Co_Status WHERE p.Co_Code='$name';");
              if($result = $db->fetch_assoc($sql))
                ...
                function  find_collection_transactions($table, $name){
                  global $db;
                  $sql = "SELECT p.*, d.*  FROM $table p LEFT JOIN loan_payment d ON d.A_Number = p.A_Number Where  p.A_Number = '$name';" or die($mysqli->error);
