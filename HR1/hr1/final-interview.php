...
$sql = "SELECT name FROM users WHERE id = '$user_id'";
  ...
  $pangalan = $result->fetch_assoc();
    $name = $pangalan['name'];
    $log = $name.' has cancel final imterview';
    logger($log);
...
if (mail($to_email, $subject, $body, $headers)) {
    ...
    $_SESSION['status'] = "Schedule cancelled";
      ...
      redirect('final-interview.php',false);
          ...
          redirect('final-interview.php?id='.$id, false);
}
...
$sql = "SELECT applicants.first_name,applicants.last_name,final_interview.*,applications.* FROM applicants";
$sql .= " JOIN initial_interview ON applicants.applicant_id = initial_interview.applicant_id";
$sql .= " JOIN final_interview ON initial_interview.applicant_id = final_interview.applicant_id";
$sql .= " JOIN applications ON final_interview.applicant_id = applications.applicant_id WHERE applications.status";
$sql .= " = 'on final interview'";
$result = $db->query($sql);
  ...
  <div class="col-md-12 mb-3">
    <div class="card">
      <div class="card-header">
        <span class="badge rounded-pill bg-success"><i class="bi bi-calendar"></i>Final interview schedules</span>
         <div class="text-end mb-3">
         <span> <a href="applicants-for-hiring.php" class="btn btn-info btn-sm pull-right"><i class="bi bi-people"></i>Qualified applicants for hiring</a></span>
         </div>
         <div class="text-end">
          <a href="final-interview-report.php" class="btn btn-info btn-sm pull-right"><i class="bi bi-file-post"></i>Print report</a></div>
         </div>
      </div>
      <div class="card-body">
       <table
           ...
           <td class="text-center">
             <div class="btn-group">
              <?php $today = date('Y-m-d');
                    ...
                    if($today < $info['date']){?>
              <form method="post" action="final-interview.php">
                <input type="hidden" name="id" value="<?php echo $info['applicant_id']; ?>">
                <button  type="submit" class="btn btn-sm btn-danger" name="cancel">
