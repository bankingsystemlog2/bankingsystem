...
function find_by_job_id($table,$id)
{
  global $db;
  $id = (int)$id;
    if(tableExists($table)){
          $sql = $db->query("SELECT * FROM {$db->escape($table)} WHERE job_id='{$db->escape($id)}' LIMIT 1");
          if($result = $db->fetch_assoc($sql))
            return $result;
          else
            return null;
...
function find_applicants(){
      global $db;
      $results = array();
      $sql = "SELECT a.first_name, a.last_name, app.job_title, app.date_of_application, app.status, app.applicant_id,app.id ";
      $sql .= "FROM applicants AS a JOIN applications as app WHERE a.applicant_id = app.applicant_id;";
      $result = find_by_sql($sql);
      return $result;
  }

function find_all_password_request() {
   global $db;
   $sql = "SELECT change_pass_request.*,employees.* FROM change_pass_request JOIN employees ON ";
   $sql .= "change_pass_request.employee_id = employees.employee_id WHERE change_pass_request.department = 'HR1' AND change_pass_request.status = 'pending'";
   $res = find_by_sql($sql);
...
/*--------------------------------------------------------------*/
 ...
 function updateLastLogIn($user_id)
   ...
   /*--------------------------------------------------------------*/
  /* Function to get monthly yop performer
  /*--------------------------------------------------------------*/

  function monthlyTop($month){
    global $db;
  $sql = "SELECT MAX(average) as average,employee_id,date FROM performance_review WHERE MONTH(date) = MONTH('{$month}') AND status = 'approved'";
$result = $db->query($sql);
...
if($row>0){
  ...
  $json = array('name' => $info['first_name']." ".$info['last_name'],'average' => $average,'date' => $date,'id' => $info['employee_id']);
}
return $json;
}
  /*--------------------------------------------------------------*/
